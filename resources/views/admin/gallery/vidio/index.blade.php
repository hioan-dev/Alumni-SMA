                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                            <h4 class="card-title">Gallery Vidio</h4>
                        </div>
                        <a href="{{route('gallery-vidio.create')}}" class="btn btn-primary btn-sm gap-2"><i
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="datatable" class="table table-striped" data-toggle="data-table">
                                ...
                                <tbody>
                                    @forelse ($vidio as $row)
                                    <tr>
                                        <td>{{$loop->iteration}}</td>
                                        <td><img src="{{asset('storage/'.$row->image)}}" alt="" width="100px"></td>
                                        <td>{{$row->title}}</td>
                                        <td>
                                            <a href="{{ route('gallery-vidio.edit', $row->id) }}"
                                                class="btn btn-warning btn-sm gap-2"><i
                                                    class="bi bi-pencil-square"></i></a>

                                                <form action="{{ route('gallery-vidio.destroy', $row->id)}}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
